...
typedef RT_MODEL_Teach_Points_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * Teach_Points(void);

/* A global buffer for storing error messages (defined in quanser_common library) */
EXTERN char _rt_error_message[512];

/*
   extern void Teach_Points_initialize();
   extern void Teach_Points_terminate();
 */
...
static void rtExtModeSingleTaskUpload(RT_MODEL * S)
{
  {                                    /* Sample time: [0.01s, 0.0s] */
    rtExtModeUpload(0, Teach_Points_M->Timing.taskTime0);
  }
}
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Teach_Points_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://Teach_Points:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Teach_Points -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
